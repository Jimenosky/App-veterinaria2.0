...
export default function UsuariosScreen() {
  ...
  const fetchUsuarios = async () => {
    try {
      setLoading(true);
      const response = await fetch('https://api-express-mysql-de-jime.onrender.com/api/v1/users', {
        headers: {
  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api-express-mysql-de-jime.onrender.com/api/v1/users/${selectedUser.id}`,
        {
          method: 'PUT',
          headers: {
